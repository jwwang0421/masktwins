...
class DACS(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        if cfg['TwoMask'] and (cfg['mask_mode'] not in ['separate', 'separateaug']): 
            self.cut_tgt = True
        else:
            self.cut_tgt = False
    ...
    def masked_feat_dist(self, f1, f2, mask=None):
        # batch size=1 or small, results can be none
        feat_diff = f1 - f2
    ...
    def forward_train(self,
        ...
        if self.cut_tgt:
            ...
            valid_pseudo_mask=valid_pseudo_mask[0:1,:]
        ...
        if not self.source_only:
            ...
            if self.cut_tgt:
                mixed_seg_weight = torch.stack((mixed_seg_weight, mixed_seg_weight), dim=0).squeeze(1)
            mix_masks = get_class_masks(gt_semantic_seg)

            for i in range(batch_size):
                strong_parameters['mix'] = mix_masks[i]
                if self.cut_tgt:
                    mix_data=torch.stack((img[i], target_img[0]))
                    mix_target=torch.stack(
                        (gt_semantic_seg[i][0], pseudo_label[0]))
                    mix_target_weight=torch.stack((gt_pixel_weight[0], pseudo_weight[0]))
                else:
                    mix_data=torch.stack((img[i], target_img[i]))
                    mix_target=torch.stack(
                        (gt_semantic_seg[i][0], pseudo_label[i]))
                    mix_target_weight=torch.stack((gt_pixel_weight[i], pseudo_weight[i]))
                ...
                _, mixed_seg_weight[i] = strong_transform(
                    strong_parameters,
                    target=mix_target_weight)
            del gt_pixel_weight,mix_data,mix_target,mix_target_weight
                    ...
                    for k1, (n1, outs) in enumerate(seg_debug.items()):
                        for k2, (n2, out) in enumerate(outs.items()):
                            if isinstance(out, dict):
                                ...
                                subplotimg(
                                axs[k2][k1],
                                **prepare_debug_out(f'{n1} {n2}', out_1[j],
                                                    means, stds))
                            else:
                                subplotimg(
                                    axs[k2][k1],
                                    **prepare_debug_out(f'{n1} {n2}', out[j] if j<len(out) else out[0],
                                                        means, stds))
