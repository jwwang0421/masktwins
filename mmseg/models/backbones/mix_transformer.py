...
# ---------------------------------------------------------------
# MaskTwins
# - Add masking on feature level (optional)
...
class MixVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        self.mask_gen = BlockMaskGenerator(mask_ratio=0.5, mask_block_size=64)
    ...
    def forward_features(self, x, feat=False, feat_level=1, count=0):
        B = x.shape[0]
        outs = []
        ...
        # stage 1
        if feat and feat_level == 1:
            if count==1:
                mask = self.mask_gen.generate_mask(x, mask_ratio=0.5)
                self.mask_one = mask
                x = x * mask
            elif count==2:
                x = x * (1.0 - self.mask_one)
        outs.append(x)
    ...
    def forward(self, x, feat=False, feat_level=1, count=0):
        x = self.forward_features(x,feat=feat,feat_level=feat_level,count=count)
        # x = self.head(x)
