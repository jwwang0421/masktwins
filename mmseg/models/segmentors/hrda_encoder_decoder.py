...
# - Add masking on different feature levels
...
class HRDAEncoderDecoder(EncoderDecoder):
    ...
    def extract_unscaled_feat(self, img, feat=False,count=0):
        if feat:
            x = self.backbone(img,feat=feat,feat_level=2, count=count)
            return x
        else:
            x = self.backbone(img)
            if self.with_neck:
    ...
    def _forward_train_features(self, img,count=None,feat=False):
        mres_feats = []
        self.decode_head.debug_output = {}
        ...
        for i, s in enumerate(self.scales):
            ...
            if self.crop_size is not None and i >= 1:
                ...
                self.decode_head.set_hr_crop_box(crop_box,count=count)
                scaled_img = crop(scaled_img, crop_box)
            if self.decode_head.debug:
                self.decode_head.debug_output[f'Img {i} Scale {s}'] = \
                    scaled_img.detach()
            mres_feats.append(self.extract_unscaled_feat(scaled_img,feat=feat,count=count))
    ...
    def forward_train(self,
        ...
        if isinstance(img,dict):
            img1= img['1']
            mres_feats1, prob_vis = self._forward_train_features(img1,count=1)
            img2= img['2']
            mres_feats2, prob_vis = self._forward_train_features(img2,count=2)
            mres_feats={'1':mres_feats1,'2':mres_feats2}
        elif feat:
            mres_feats1, prob_vis = self._forward_train_features(img,feat=feat,count=1)
            mres_feats2, prob_vis = self._forward_train_features(img,feat=feat,count=2)
            mres_feats={'1':mres_feats1,'2':mres_feats2}
        else:
